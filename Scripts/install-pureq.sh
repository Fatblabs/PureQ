...
cleanup() {
  ...
  if [[ "$exit_status" -ne 0 ]]; then
    ...
    if [[ "$ROLLBACK_APP_PENDING" == "1" && -n "$ROLLBACK_APP_BACKUP" && ! -e "$APP_INSTALL_PATH" && -e "$ROLLBACK_APP_BACKUP" ]]; then
      run_as_root_no_prompt /bin/mv "$ROLLBACK_APP_BACKUP" "$APP_INSTALL_PATH" >/dev/null 2>&1 || true
    fi
    if [[ "$ROLLBACK_DRIVER_PENDING" == "1" && -n "$ROLLBACK_DRIVER_BACKUP" && ! -d "$DRIVER_INSTALL_PATH" && -d "$ROLLBACK_DRIVER_BACKUP" ]]; then
      run_as_root_no_prompt /bin/mv "$ROLLBACK_DRIVER_BACKUP" "$DRIVER_INSTALL_PATH" >/dev/null 2>&1 || true
    fi
  fi
  for path in "${TEMP_PATHS[@]}"; do
    ...
    [[ -n "$path" ]] && run_as_root_no_prompt /bin/rm -rf "$path" >/dev/null 2>&1 || true
  done
  exit "$exit_status"
}
...
install_uninstaller() {
  run_as_root /bin/mkdir -p "$SUPPORT_DIR"
  run_as_root /usr/bin/env COPYFILE_DISABLE=1 /usr/bin/ditto --norsrc --noextattr "$ROOT_DIR/Scripts/uninstall-pureq.sh" "$SUPPORT_DIR/uninstall-pureq.sh"
  run_as_root /bin/chmod 755 "$SUPPORT_DIR/uninstall-pureq.sh"
...
WRAPPER
  run_as_root /usr/bin/install -m 755 "$wrapper" "$UNINSTALL_COMMAND_PATH"
  /bin/rm -f "$wrapper"
}

install_recovery_helper() {
  [[ "$INSTALL_RECOVERY_HELPER" == "1" ]] || return 0

  run_as_root /bin/mkdir -p "$SUPPORT_DIR" "$(dirname "$RECOVERY_AGENT_PATH")"
  run_as_root /usr/bin/env COPYFILE_DISABLE=1 /usr/bin/ditto --norsrc --noextattr "$RECOVERY_HELPER_SRC" "$SUPPORT_DIR/PureQAudioRecovery"
  run_as_root /bin/chmod 755 "$SUPPORT_DIR/PureQAudioRecovery"
...
AGENT
  run_as_root /usr/bin/install -m 644 "$agent" "$RECOVERY_AGENT_PATH"
  /bin/rm -f "$agent"
  ...
  if [[ -n "$console_user" && "$console_user" != "root" ]]; then
    ...
    if [[ -n "$console_uid" ]]; then
      run_as_root /bin/launchctl bootout "gui/$console_uid" "$RECOVERY_AGENT_PATH" >/dev/null 2>&1 || true
      run_as_root /bin/launchctl bootstrap "gui/$console_uid" "$RECOVERY_AGENT_PATH" >/dev/null 2>&1 || true
      run_as_root /bin/launchctl kickstart -k "gui/$console_uid/Sean-s-Apps.PureQ.AudioRecovery" >/dev/null 2>&1 || true
    fi
  fi
}
...
install_app_payload() {
  ...
  safe_rm_rf "$stage"
  run_as_root /usr/bin/env COPYFILE_DISABLE=1 /usr/bin/ditto --norsrc --noextattr "$APP_SRC" "$stage"
  run_as_root /usr/bin/xattr -dr com.apple.quarantine "$stage" 2>/dev/null || true

  if [[ -e "$APP_INSTALL_PATH" ]]; then
    safe_rm_rf "$previous"
    ROLLBACK_APP_BACKUP="$previous"
    ROLLBACK_APP_PENDING=1
    run_as_root /bin/mv "$APP_INSTALL_PATH" "$previous"
  fi
  run_as_root /bin/mv "$stage" "$APP_INSTALL_PATH"
  ROLLBACK_APP_PENDING=0
}
...
install_driver_payload() {
  ...
  safe_rm_rf "$stage"
  run_as_root /bin/mkdir -p "$(dirname "$DRIVER_INSTALL_PATH")" "$SUPPORT_DIR/Backups"
  run_as_root /usr/bin/env COPYFILE_DISABLE=1 /usr/bin/ditto --norsrc --noextattr "$DRIVER_SRC" "$stage"
  run_as_root /usr/sbin/chown -R root:wheel "$stage"
  run_as_root /bin/chmod -R go-w "$stage"
  run_as_root /usr/bin/xattr -cr "$stage" 2>/dev/null || true

  if [[ -e "$DRIVER_INSTALL_PATH" ]]; then
    ROLLBACK_DRIVER_BACKUP="$backup"
    ROLLBACK_DRIVER_PENDING=1
    run_as_root /bin/mv "$DRIVER_INSTALL_PATH" "$backup"
  fi

  run_as_root /bin/mv "$stage" "$DRIVER_INSTALL_PATH"
  ROLLBACK_DRIVER_PENDING=0
}
